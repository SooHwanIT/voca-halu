...
const MODELS = [
  { id: '@cf/deepgram/aura-1', input: t => ({ text: t, speaker: 'luna' }), type: 'audio/mpeg' },
...
async function tts(env, text) {
  ...
  for (const m of MODELS) {
    try {
      const res = await env.AI.run(m.id, m.input(text));
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/tts') {
      ...
      const key = new Request(new URL('/api/tts?v=1&t=' + encodeURIComponent(text.toLowerCase()), url.origin).toString());
      const hit = await cache.match(key);
      if (hit) return hit;
      try {
        const r = await tts(env, text);
